...
map.setView(new L.LatLng(40.7619, -73.9763), 13).addLayer(cloudmade);
...
if(checkins.count >0){
	...
	for (var i=0;i<checkins.count;i++){
		var site = items[i],
			...
			catImg = site.venue.categories[0].icon.prefix;
		
		catImg = catImg.replace('_v2','');
		catImg = catImg.replace('_','');
		catImg = catImg.replace(' ','');
		catImg = catImg+'.png';
		...
		//Generate layers
		if(typeof mapLayers['mySite'] != "object")mapLayers['mySite'] = new L.LayerGroup();
		mapLayers['mySite'].addLayer(marker);
		
		map.addLayer(mapLayers['mySite']);
...
(function(){//Bubbling touch handlers
